...
import type { CreateMapboxActions } from '@libs/types'
...
export function useCreateMapbox(
  elRef: MaybeRef<HTMLElement | string>,
  props: CreateMapboxProps = { container: '', style: '' }
) {
  const mapInstance = shallowRef<Map | null>(null)
  const mapStatus = ref<MapboxStatus>(MapboxStatus.NotLoaded)
  const { register, ...options } = props
  const mapOptions = ref<MapOptions>(options)
  ...
  function setStyle(style: string | StyleSpecification) {
