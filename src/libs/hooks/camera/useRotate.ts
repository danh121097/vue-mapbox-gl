...
export function useResetNorth(map: ShallowRef<Nullable<Map>>, options?: AnimationOptions) {
  const aimationOptions = ref<Undefinedable<AnimationOptions>>(options)

  watchEffect(() => {
    if (map.value && aimationOptions.value) map.value.resetNorth(aimationOptions.value)
  })
...
export function useResetNorthPitch(map: ShallowRef<Nullable<Map>>, options?: AnimationOptions) {
  const aimationOptions = ref<Undefinedable<AnimationOptions>>(options)

  watchEffect(() => {
    if (map.value && aimationOptions.value) map.value.resetNorthPitch(aimationOptions.value)
  })
...
export function useSnapToNorth(map: ShallowRef<Nullable<Map>>, options?: AnimationOptions) {
  const aimationOptions = ref<Undefinedable<AnimationOptions>>(options)

  watchEffect(() => {
    if (map.value && aimationOptions.value) map.value.snapToNorth(aimationOptions.value)
  })
